...
if(isset($_SESSION["account_id"])){
  ...
  try{
    $query = $pdo->prepare("INSERT INTO `goal` VALUES(:task_id, {$_SESSION["account_id"]});");
    $query->execute(array(":task_id" => $task_id));
    
    if($query->rowCount() == 0){
      error_log("[{$logTime}] '{$_SERVER["PHP_SELF"]}': insert fail -> ({$task_id}, '{$_SESSION["account_id"]}')\n", 3, "error_log");
      echo "fail";
    }
    else{
    ...
    error_log("[{$logTime}] '{$_SERVER["PHP_SELF"]}': parameters -> ({$task_id}, '{$_SESSION["account_id"]}')\n", 3, "error_log");
    http_response_code(500);
    die("Internal Server Error");
  }
  $pdo = null;
