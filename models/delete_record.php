...
if(isset($_SESSION["account_id"]) && isset($_REQUEST["record_id"])){
  ...
  try{
    $query = $pdo->prepare("DELETE FROM `records` WHERE `id` = :record_id AND `account_id` = :account_id;");
    $query->execute(array(":record_id" => $_REQUEST["record_id"], ":account_id" => $_SESSION["account_id"]));
