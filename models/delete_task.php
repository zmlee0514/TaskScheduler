...
if(isset($_SESSION["account_id"]) && isset($_REQUEST["task_id"])){
  ...
  try{
    $query = $pdo->prepare("UPDATE `tasks` SET `archive` = 1 WHERE `id` = :task_id AND `account_id` = :account_id;");
    $query->execute(array(":task_id" => $_REQUEST["task_id"], ":account_id" => $_SESSION["account_id"]));

    if($query->rowCount() == 0){
      error_log("[{$logTime}] '{$_SERVER["PHP_SELF"]}': delete fail -> `id` = {$_REQUEST["task_id"]} AND `account_id` = {$_SESSION["account_id"]}\n", 3, "error_log");
      echo "fail";
    }
    else echo "success";
  }
    ...
    error_log("[{$logTime}] '{$_SERVER["PHP_SELF"]}': parameters -> `id` = {$_REQUEST["task_id"]} AND `account_id` = {$_SESSION["account_id"]}\n", 3, "error_log");
    http_response_code(500);
    die("Internal Server Error");
  }
  $pdo = null;
