...
if(isset($_SESSION["account_id"])){
  ...
  try{
    $query = $pdo->prepare("UPDATE `category` SET `name` = :name, `color_id` = :color_id WHERE `id` = :category_id AND `account_id` = :account_id;");
    $query->execute(array(":name" => $category_name, ":color_id" => $color_index, ":category_id" => $_REQUEST["category_id"], ":account_id" => $_SESSION["account_id"]));
    
    if($query->rowCount() == 0){
      error_log("[{$logTime}] '{$_SERVER["PHP_SELF"]}': update fail -> SET `name` = {$category_name}, `color_id` = {$color_index} WHERE `id` = {$_REQUEST["category_id"]} AND `account_id` = {$_SESSION["account_id"]}\n", 3, "error_log");
      echo "fail";
    }
    else{
    ...
    error_log("[{$logTime}] '{$_SERVER["PHP_SELF"]}': statement -> SET `name` = {$category_name}, `color_id` = {$color_index} WHERE `id` = {$_REQUEST["category_id"]} AND `account_id` = {$_SESSION["account_id"]}\n", 3, "error_log");
    http_response_code(500);
    die("Internal Server Error");
  }
  $pdo = null;
