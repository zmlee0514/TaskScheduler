...
if(isset($_SESSION["account_id"])){
  ...
  try{
    $stmt = "SELECT t.`id`,t.`name`,t.`category_id`,t.`description`,t.`deadline`,t.`predict`,c.`color_id`,c.`name` AS category 
      FROM `tasks` AS t LEFT JOIN `category` AS c ON t.`category_id` = c.`id` 
      WHERE t.`account_id` = {$_SESSION["account_id"]} AND t.`archive` = 0 
      ORDER BY t.`deadline` DESC, t.`id`;";
